import bpy
import numpy as np
from .classification_files.shader_type import Type1,Type2,Type3
from .functions.get_data import get_frametime,is_file_path_exists,get_file_path
...
def create_node_material(texture_paths, mat_name,filename):
    ...
    path = bpy.context.scene.material_blend_path
    if ':' not in mat_name:
...
def fac(origin, position, element, vertices, faces, vertices_dict, directions, has_air, rotation,rotation_matrix=None):
    ...
    for direction, condition in directions_map.items():
        if condition:
            ...
            coords = [(round(coord[0], 2), round(coord[1], 2), round(coord[2], 2)) for coord in coords]
            for coord in coords:
            ...
            if tuple(element['from']) in v_from and tuple(element['to']) in v_to:
                ...
                for face_direction, face_visible in face_visibility.items():
                    if face_visible and face_direction in element['faces']:
                        ...
                        coords = rot(origin, position, coords,Rotation,rotation_matrix)
                        coords = [(round(coord[0], 2), round(coord[1], 2), round(coord[2], 2)) for coord in coords]  
                        texture = element['faces'][face_direction].get('texture')
                        ...
                        try:
                            if texture_list[faces.index([vertices_dict[coords[3]],vertices_dict[coords[2]],vertices_dict[coords[0]],vertices_dict[coords[1]]])] != texture:
                                texture_list[faces.index([
                                    vertices_dict[coords[3]],
                                    vertices_dict[coords[2]],
                                    vertices_dict[coords[0]],
                                    vertices_dict[coords[1]]
...
def get_or_create_material(texture,filename):
    ...
    if mat_name not in bpy.data.materials:
        if isinstance(texture, str):
            ...
            mat = create_node_material([get_file_path(str(texture[0]), "t"),get_file_path(str(texture[1]), "t")], str(texture[0]),filename)
